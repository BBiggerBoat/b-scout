# B-Atlas Cloudflare persistence
...
Cloudflare D1 Free includes point-in-time recovery. B-Atlas also exposes `GET /api/admin/backup`, which returns the complete moderation state and published overlay as JSON for a manual off-platform backup.

Attachments are stored in Workers KV. They are not included in the JSON backup; the checked-in contribution metadata keeps their attachment IDs. For a hobby/prototype deployment this is intentionally simple. If B-Atlas gains traction, move attachment storage to R2 and add scheduled off-platform object backups.
